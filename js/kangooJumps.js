...
.then( (resultado) => {
    ...
    actKangooJumps (resultado);
    
});

const actKangooJumps = () => {
    entrenamientos.map((entrenamientos) => {
        entrenamientos.nombre == "Kangoo Jumps" && kangooJumps.push(new Actividad(entrenamientos)); 
    });
    
    kangooJumps.map((actividad) => {
        const contenedor = document.getElementById("contenedor-kangooJumps");
    ...
    const obtenerProducto = (id) => {
        return kangooJumps.find(actividad => actividad.id == id);
    }
